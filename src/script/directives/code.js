...
angular
    ...
    .directive('codeContainer', function () {
        return {
            ...
            scope: true,
            transclude: true,
            template: '<h2 ng-repeat="tab in code.tabs" ng-class="{active: $index == code.currentTab}" ng-click="loadTab($index)"><span class="tab"></span>{{ tab }}</h2><h2><span class="tab"></span><input type="file">+</h2><h2 title="Delete the current tab." ng-click="removeTab()"><span class="tab"></span>x</h2>' +
